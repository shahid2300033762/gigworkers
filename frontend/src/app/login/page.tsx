...
export default function LoginPage() {
        ...
        async function checkSession() {
            try {
                ...
                if (session && !error) {
                    ...
                    if (user && !userError) {
                        // Valid session — check if onboarding is done
                        ...
                        // Session is stale/invalid — sign out and show login
                        try { await supabase.auth.signOut(); } catch (e) {}
                    }
                }
            } catch (err) {
                console.error("Auth check failed in login:", err);
            } finally {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const worker = user ? await getWorkerProfile(user.id) : null;
            if (worker) {
                router.push('/dashboard');
            } else {
                router.push('/onboarding');
    ...
    return (
        <div className="min-h-screen bg-background-light flex flex-col items-center justify-center p-4 relative font-display text-primary">
            ...
            <div className="absolute top-0 left-0 w-full h-full overflow-hidden pointer-events-none z-0">
                <div className="absolute -top-[20%] -right-[10%] w-[50%] h-[50%] bg-accent-teal/10 blur-[100px] rounded-full"></div>
                <div className="absolute -bottom-[20%] -left-[10%] w-[50%] h-[50%] bg-accent-purple/10 blur-[100px] rounded-full"></div>
            </div>

            <div className="w-full max-w-md z-10">
                <div className="flex flex-col items-center justify-center mb-10 cursor-pointer" onClick={() => router.push('/')}>
                    <div className="bg-primary p-2.5 rounded-xl mb-4 shadow-lg shadow-primary/20">
                        <Shield className="text-white w-8 h-8" />
                    </div>
                    <h1 className="text-3xl font-black tracking-tight">Welcome Back</h1>
                    <p className="text-slate-500 mt-2 text-center">Enter your details and get back to your dashboard.</p>
                </div>
                ...
                <div className="glass-card bg-white/70 backdrop-blur-xl rounded-2xl p-8 shadow-2xl shadow-primary/5 border border-white">
                    <form onSubmit={handleLogin} className="space-y-6">
                        ...
                        <button
                            type="submit"
                            disabled={loading}
                            className="w-full bg-primary text-white font-bold py-4 rounded-xl hover:bg-slate-800 hover:shadow-lg hover:shadow-primary/20 transition-all flex items-center justify-center gap-2 group mt-2"
                        >
                            {loading ? (
